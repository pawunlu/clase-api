...
En `app/dependencies.php` registramos una nueva dependencia en el contenedor llamada `db` y a la misma le asignamos una función anónima que se encargará de generar la instancia de PDO la primera vez que se requiera la depdencia. Para ello, en dicho archivo, agregamos en el array de depdencias que se está registrando lo siguiente:
...
Esto creará la tabla en la base. Obéervese que si se intenta aplicar nuevamente la migración, phinx no realizará ninguna acción dado que lleva un control de las migraciones ya aplicadas
